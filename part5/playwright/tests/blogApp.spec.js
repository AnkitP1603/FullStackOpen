  ...
  describe("When logged in", () => {
    ...
    test("a new blog can be created", async ({ page }) => {
      createBlog(
        ...
        "playwright author",
        "http://playwright.test/blog",
      );

      const successDiv = page.locator(".notification");
      await expect(successDiv).toContainText(
        "A new blog a blog created by playwright by playwright author added",
      );
      await expect(
    ...
    test("user can delete their own blog", async ({ page }) => {
      ...
      await expect(
        page.getByText("deletable blog delete author"),
      ).not.toBeVisible();
    });

    test("user can see delete button on their own blogs", async ({
      page,
      request,
    }) => {
      createBlog(
