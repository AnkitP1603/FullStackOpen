...
const App = () => {
  ...
  const handleFormSubmit = (event) => {
    ...
    if (exists) {
      const pos = window.confirm(`${nameToAdd} is already added to phonebook, replace the old number with a new one?`)
      if(pos) {
        const personToUpdate = persons.find(p => p.name.toLowerCase() === nameToAdd.toLowerCase()) 
  ...
  return (
    <div>
      ...
      <h2>Add a new</h2>
      <PersonForm newName={newName} handleNameChange={handleNameChange} newNumber={newNumber} handleNumberChange={handleNumberChange} handleFormSubmit={handleFormSubmit}/>
